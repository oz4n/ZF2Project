...
use Search\Index\SegmentWriter\StreamWriter;
use Search\Lucene\Exception;
use Search\Lucene\Field;
use Search\Index\TermsPriorityQueue;
...
class SegmentMerger
{
     ...
     *
     * @param \Search\Storage\Directory $directory
     * @param string $name
     */
    public function __construct($directory, $name)
    {
      
        $this->_writer = new StreamWriter($directory, $name);
    }
    ...
    public function merge()
    {
        if ($this->_mergeDone) {
            ...
            throw new Exception('Merge is already done.');
        }

        if (count($this->_segmentInfos) < 1) {
            
            throw new Exception('Wrong number of segments to be merged ('
        ...
        foreach ($this->_segmentInfos as $segName => $segmentInfo) {
            ...
            for ($count = 0; $count < $segmentInfo->count(); $count++) {
                ...
                for ($count2 = 0; $count2 < $fieldCount; $count2++) {
                    ...
                    if (!($bits & 2)) { // Text data
                        $storedFields[] = new Field($fieldInfo->name, $fdtFile->readString(), 'UTF-8', true, $fieldInfo->isIndexed, $bits & 1);
                    } else {            // Binary data
                        $storedFields[] = new Field($fieldInfo->name, $fdtFile->readBinary(), '', true, $fieldInfo->isIndexed, $bits & 1, true);
                    }
                }
