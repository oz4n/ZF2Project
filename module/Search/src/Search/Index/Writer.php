...
use Search\Lucene\LockManager;
use Search\Lucene\Lucene;
use Search\Storage\Directory;
use Search\Lucene\Exception;
use Search\Index\SegmentWriter\DocumentWriter;
use Search\Lucene\Document;
use Search\Index\SegmentMerger;
use Search\Index\SegmentInfo;
/**
 * @category   Zend
 * @package    Zend_Search_Lucene
 * @subpackage Index
...
class Writer
{
     ...
     * @todo Implement DirectoryRAM and Zend_Search_Lucene_Storage_FileRAM to use it for
    ...
    /**
     * Array of SegmentInfo objects for this index.
     *
     * It's a reference to the corresponding Lucene::$_segmentInfos array
     *
    ...
    public static function createIndex(Directory $directory, $generation, $nameCount)
    {
        if ($generation == 0) {
            ...
            $segmentsFile = $directory->createFile(Lucene::getSegmentFileName($generation));
    ...
    public function addDocument(Document $document)
    {
        ...
        if ($this->_currentSegment === null) {
            $this->_currentSegment = new DocumentWriter($this->_directory, $this->_newSegmentName());
        }
    ...
    private function _updateSegments()
    {
        ...
        $generation = Lucene::getActualGeneration($this->_directory);
        $segmentsFile = $this->_directory->getFileObject(Lucene::getSegmentFileName($generation), false);
        $newSegmentFile = $this->_directory->createFile(Lucene::getSegmentFileName(++$generation), false);
        ...
        try {
            // Write format marker
            if ($this->_targetFormatVersion == Lucene::FORMAT_2_1) {
                $newSegmentFile->writeInt((int) 0xFFFFFFFD);
            } else if ($this->_targetFormatVersion == Lucene::FORMAT_2_3) {
            ...
            if ($format == (int) 0xFFFFFFFF) {
                $srcFormat = Lucene::FORMAT_PRE_2_1;
            } else if ($format == (int) 0xFFFFFFFD) {
                $srcFormat = Lucene::FORMAT_2_1;
            } else if ($format == (int) 0xFFFFFFFC) {
                $srcFormat = Lucene::FORMAT_2_3;
            } else {
            ...
            for ($count = 0; $count < $segmentsCount; $count++) {
                ...
                if ($srcFormat == Lucene::FORMAT_PRE_2_1) {
                    ...
                    if ($srcFormat == Lucene::FORMAT_2_3) {
                ...
                if (!in_array($segName, $this->_segmentsToDelete)) {
                    ...
                    if ($this->_targetFormatVersion == Lucene::FORMAT_2_3) {
                        ...
                        throw new Exception('Index conversion to lower format version is not supported.');
                    }
        ...
        if (LockManager::escalateReadLock($this->_directory)) {
            ...
            foreach ($this->_directory->fileList() as $file) {
                if ($file == 'deletable') {
                    ...
                    if ($file != Lucene::getSegmentFileName($generation)) {
                        $filesToDelete[] = $file;
