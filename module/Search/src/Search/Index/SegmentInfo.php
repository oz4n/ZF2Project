...
use Search\Search\Similarity;
use Search\Index\FieldInfo;
use Search\Index\TermInfo;
use Search\Lucene\Exception;
use Search\Storage\Directory;
use Search\Index\DictionaryLoader;
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SegmentInfo implements IndexInterface
{
     ...
     *
     * Array of arrays (Term objects are represented as arrays because
     * of performance considerations)
     * [0] -> $termValue
     * [1] -> $termFieldNum
     *
     * Corresponding TermInfo object stored in the $_termDictionaryInfos
     *
     * @var array
     */
     ...
     *
     * Array of arrays (TermInfo objects are represented as arrays because
     * of performance considerations)
     * [0] -> $docFreq
     * [1] -> $freqPointer
     * [2] -> $proxPointer
    ...
    /**
     * Segment fields. Array of FieldInfo objects for this segment
     *
     * @var array
     */
    private $_fields;
    ...
    public function __construct(Directory $directory, $name, $docCount, $delGen = 0, $docStoreOptions = null, $hasSingleNormFile = false, $isCompound = null)
    {
        ...
        for ($count = 0; $count < $fieldsCount; $count++) {
            ...
            $this->_fields[$count] = new FieldInfo($fieldName, $fieldBits & 0x01 /* field is indexed */, $count, $fieldBits & 0x02 /* termvectors are stored */, $fieldBits & 0x10 /* norms are omitted */, $fieldBits & 0x20 /* payloads are stored */);
            if ($fieldBits & 0x10) {
    ...
    public function openCompoundFile($extension, $shareHandler = true)
    {
        if (($extension == '.fdx' || $extension == '.fdt') && $this->_usesSharedDocStore) {
            ...
            if (!isset($this->_sharedDocStoreOptions['files'][$fdxFName])) {
                ...
                throw new Exception('Shared doc storage segment compound file doesn\'t contain '
                . $fdxFName . ' file.');
            }
        ...
        if (!isset($this->_segFiles[$filename])) {
            ...
            throw new Exception('Segment compound file doesn\'t contain '
            . $filename . ' file.');
        }
    ...
    public function compoundFileLength($extension)
    {
        if (($extension == '.fdx' || $extension == '.fdt') && $this->_usesSharedDocStore) {
            ...
            if (!isset($this->_sharedDocStoreOptions['fileSizes'][$filename])) {
                ...
                throw new Exception('Shared doc store compound file doesn\'t contain '
                . $filename . ' file.');
            }
        ...
        if (!isset($this->_segFileSizes[$filename])) {
            ...
            throw new Exception('Index compound file doesn\'t contain '
            . $filename . ' file.');
        }
    ...
    public function getTermInfo(Term $term)
    {
        ...
        if ($tiVersion != (int) 0xFFFFFFFE /* pre-2.1 format */ &&
            ...
            throw new Exception('Wrong TermInfoFile file format');
        }
    ...
    public function termDocs(Term $term, $shift = 0, $docsFilter = null)
    {
        ...
        if ($docsFilter !== null) {
            if (!$docsFilter instanceof DocsFilter) {
                ...
                throw new Exception('Documents filter must be an instance of DocsFilter or null.');
            }
...
// ---------------------------------------------------------------
    ...
    private function _loadNorm($fieldNum)
    {
        if ($this->_hasSingleNormFile) {
            ...
            if ($header != 'NRM' || $headerFormatVersion != (int) 0xFF) {
                ...
                throw new Exception('Wrong norms file format.');
            }
    ...
    public function writeChanges()
    {
        ...
        if (!$this->_deletedDirty) {
            ...
            if ($latestDelGen == $this->_delGen) {
                ...
                throw new Exception('Delete file processing workflow is corrupted for the segment \'' . $this->_name . '\'.');
            }
        }
    ...
    public function resetTermsStream(/** $startId = 0, $mode = self::SM_TERMS_ONLY */)
    {
        ...
        switch ($mode) {
            ...
            default:
                ...
                throw new Exception('Wrong terms scaning mode specified.');
                break;
        }
    ...
    public function skipTo(Term $prefix)
    {
        ...
        $this->_lastTerm = new Term($prevTerm[1] /* text */, ($prevTerm[0] == -1) ? '' : $this->_fields[$prevTerm[0] /* field */]->name);
        $this->_lastTermInfo = new TermInfo($prevTermInfo[0] /* docFreq */, $prevTermInfo[1] /* freqPointer */, $prevTermInfo[2] /* proxPointer */, $prevTermInfo[3] /* skipOffset */);
