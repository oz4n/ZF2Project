...
abstract class SegmentWriter
{
    ...
    public function addField(Field $field)
    {
        if (!isset($this->_fields[$field->name])) {
            ...
            $this->_fields[$field->name] = new FieldInfo($field->name, $field->isIndexed, $fieldNumber, $field->storeTermVector);
    ...
    public function addFieldInfo(FieldInfo $fieldInfo)
    {
        if (!isset($this->_fields[$fieldInfo->name])) {
            $fieldNumber = count($this->_fields);
            $this->_fields[$fieldInfo->name] = new FieldInfo($fieldInfo->name, $fieldInfo->isIndexed, $fieldNumber, $fieldInfo->storeTermVector);
     ...
     *
     * @param array $storedFields array of Field objects
     */
...
// not supported yet            0x08 /* term offsets are stored with the term vectors */   |
     ...
     */
    protected function _dumpTermDictEntry(File $dicFile, &$prevTerm, Term $term, &$prevTermInfo, TermInfo $termInfo)
    {
