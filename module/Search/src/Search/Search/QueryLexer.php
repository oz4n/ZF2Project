...
class QueryLexer extends FSM
{
    ...
    public function addQuerySyntaxLexeme()
    {
        ...
        if (strpos(self::QUERY_DOUBLECHARLEXEME_CHARS, $lexeme) !== false) {
            ...
            if ($this->_queryStringPosition == count($this->_queryString) ||
                ...
                throw new Exception('Two chars lexeme expected. ' . $this->_positionMsg());
            }
        ...
        $token = new QueryToken(
                QueryToken::TC_SYNTAX_ELEMENT, $lexeme, $this->_queryStringPosition);
        ...
        if ($token->type == QueryToken::TT_FIELD_INDICATOR) {
            ...
            if ($token === null || $token->type != QueryToken::TT_WORD) {
                ...
                throw new Exception('Field mark \':\' must follow field name. ' . $this->_positionMsg());
            }
    ...
    public function addLexemeModifier()
    {
        $this->_lexemes[] = new QueryToken(
                QueryToken::TC_SYNTAX_ELEMENT, $this->_queryString[$this->_queryStringPosition], $this->_queryStringPosition);
    }
    ...
    {
        $this->_lexemes[] = new QueryToken(
                QueryToken::TC_WORD, $this->_currentLexeme, $this->_queryStringPosition - 1);
    ...
    {
        $this->_lexemes[] = new QueryToken(
                QueryToken::TC_PHRASE, $this->_currentLexeme, $this->_queryStringPosition);
    ...
    {
        $this->_lexemes[] = new QueryToken(
                QueryToken::TC_NUMBER, $this->_currentLexeme, $this->_queryStringPosition - 1);
        $this->_currentLexeme = '';
    }
    ...
    public function lexModifierErrException()
    {
        ...
        throw new Exception('Lexeme modifier character can be followed only by number, white space or query syntax element. ' . $this->_positionMsg());
    }

    public function quoteWithinLexemeErrException()
    {
        
        throw new Exception('Quote within lexeme must be escaped by \'\\\' char. ' . $this->_positionMsg());
    }

    public function wrongNumberErrException()
    {
        
        throw new Exception('Wrong number syntax.' . $this->_positionMsg());
    }
