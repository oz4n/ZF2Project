...
class Fuzzy extends Query
{
    ...
    /**
     * Lucene_Search_Query_Wildcard constructor.
     *
     ...
     * @throws Exception
     */
    public function __construct(IndexTerm $term, $minimumSimilarity = self::DEFAULT_MIN_SIMILARITY, $prefixLength = null)
    {
        if ($minimumSimilarity < 0) {

            throw new Exception('minimumSimilarity cannot be less than 0');
        }
        if ($minimumSimilarity >= 1) {

            throw new Exception('minimumSimilarity cannot be greater than or equal to 1');
        }
        if ($prefixLength < 0) {

            throw new Exception('prefixLength cannot be less than 0');
        }
    ...
    public function rewrite(LuceneInterface $index)
    {
        ...
        $prefix = IndexTerm::getPrefix($this->_term->text, $this->_prefixLength);
        $prefixByteLength = strlen($prefix);
        $prefixUtf8Length = IndexTerm::getLength($prefix);

        $termLength = IndexTerm::getLength($this->_term->text);
        ...
        foreach ($fields as $field) {
            ...
            if ($prefix != '') {
                $index->skipTo(new IndexTerm($prefix, $field));
                ...
                $index->skipTo(new IndexTerm('', $field));
        ...
        if (count($this->_matches) == 0) {
            ...
            foreach ($this->_matches as $id => $matchedTerm) {
                $subquery = new QueryTerm($matchedTerm);
    ...
    public function optimize(LuceneInterface $index)
    {
        ...
        throw new Exception('Fuzzy query should not be directly used for search. Use $query->rewrite($index)');
    }
