...
use Search\Search\Query\Term;
use Search\Search\Query\MultiTerm;
use Search\Search\Weight\Boolean as WeightBoolean;
...
class Boolean extends Query
{
     ...
     *
     * @param array $subqueries    Array of Query objects
    ...
    /**
     * Add a $subquery (Query) to this query.
     *
    ...
    public function optimize(LuceneInterface $index)
    {
        ...
        foreach ($subqueries as $id => $subquery) {
            if ($subquery instanceof QueryEmpty) {
                if ($signs[$id] === true) {
    ...
    {
        $this->_weight = new WeightBoolean($this, $reader);
        return $this->_weight;
    }
     ...
     * @return float
     */
    public function _nonConjunctionScore($docId, LuceneInterface $reader)
    {
        if ($this->_coord === null) {
            $this->_coord = array();
