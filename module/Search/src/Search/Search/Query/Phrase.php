...
use Search\Search\Weight\Phrase as WeightPhrase;
...
class Phrase extends Query
{
    ...
    public function __construct($terms = null, $offsets = null, $field = null)
    {
        ...
        if (is_array($terms)) {
            ...
            foreach ($terms as $termId => $termText) {
                $this->_terms[$termId] = ($field !== null) ? new Term($termText, $field) : new Term($termText);
            }
        } else if ($terms === null) {
            $this->_terms = array();
        } else {

            throw new Exception('terms argument must be array of strings or null');
        }

        if (is_array($offsets)) {
            if (count($this->_terms) != count($offsets)) {

                throw new Exception('terms and offsets arguments must have the same size.');
            }
            ...
            throw new Exception('offsets argument must be array of strings or null');
        }
    }
     ...
     */
    public function addTerm(Term $term, $position = null)
    {
        if ((count($this->_terms) != 0) && (end($this->_terms)->field != $term->field)) {

            throw new Exception('All phrase terms must be in the same field: ' . $term->field . ':' . $term->text);
        }
    ...
    public function rewrite(LuceneInterface $index)
    {
        if (count($this->_terms) == 0) {
            ...
            foreach ($index->getFieldNames(true) as $fieldName) {
                ...
                foreach ($this->_terms as $termId => $term) {
                    $qualifiedTerm = new Term($term->text, $fieldName);
    ...
    public function optimize(LuceneInterface $index)
    {
        ...
        if (count($this->_terms) == 1) {
            ...
            $optimizedQuery = new QueryTerm(reset($this->_terms));
    ...
    public function createWeight(LuceneInterface $reader)
    {
        ...
        $this->_weight = new WeightPhrase($this, $reader);
        return $this->_weight;
    }
     ...
     * @return float
     */
    public function _sloppyPhraseFreq($docId, LuceneInterface $reader)
    {
        $freq = 0;
