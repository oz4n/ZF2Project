...
class Wildcard extends Query
{
    ...
    /**
     * Query Wildcard constructor.
     *
     * @param Term $pattern
     */
    public function __construct(Term $pattern)
    {
    ...
    public function rewrite(LuceneInterface $index)
    {
        ...
        if ($prefixLength < self::$_minPrefixLength) {
            ...
            throw new Exception('At least ' . self::$_minPrefixLength . ' non-wildcard characters are required at the beginning of pattern.');
        }
    ...
    {        
        throw new Exception('Wildcard query should not be directly used for search. Use $query->rewrite($index)');
    }
    ...
    public function getQueryTerms()
    {
        if ($this->_matches === null) {
            ...
            throw new Exception('Search has to be performed first to get matched terms');
        }
