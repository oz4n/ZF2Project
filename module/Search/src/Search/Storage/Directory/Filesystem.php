...
use Search\Storage\File\Filesystem as FileFilesystem;
/**
...
class Filesystem extends Directory
{
    ...
    /**
     * Cache for FileFilesystem objects
     * Array: filename => File object
     *
     * @var array
     * @throws Exception
     */
    ...
    public function __construct($path)
    {
        if (!is_dir($path)) {
            if (file_exists($path)) {
                throw new Exception('Path exists, but it\'s not a directory');
            } else {
                if (!self::mkdirs($path)) {
    ...
    public function createFile($filename)
    {
        ...
        $this->_fileHandlers[$filename] = new FileFilesystem($this->_dirPath . '/' . $filename, 'w+b');
    ...
    public function deleteFile($filename)
    {
        ...
        if (!@unlink($this->_dirPath . '/' . $filename)) {
            ...
            throw new Exception('Can\'t delete file: ' . $php_errormsg);
        }
        ini_set('track_errors', $trackErrors);
    }
    ...
    /**
     * Returns a File object for a given $filename in the directory.
     *
    ...
    public function getFileObject($filename, $shareHandler = true)
    {
        ...
        if (!$shareHandler) {
            return new FileFilesystem($fullFilename);
        }
        ...
        $this->_fileHandlers[$filename] = new FileFilesystem($fullFilename);
        return $this->_fileHandlers[$filename];
    }
