...
class StopWords extends TokenFilter
{
     ...
     * @throws Exception When the file doesn`t exists or is not readable.
     */
    public function loadFromFile($filepath = null)
    {
        if (!$filepath || !file_exists($filepath)) {
            throw new Exception('You have to provide valid file path');
        }
        $fd = fopen($filepath, "r");
        if (!$fd) {
        ...
        if (!fclose($fd)) {
            ...
            throw new Exception('Cannot close file ' . $filepath);
        }
    }
